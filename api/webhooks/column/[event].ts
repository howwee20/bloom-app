...
const schemas = {
  ...
  ach_event: z.object({
    ...
    status: z.enum(['pending', 'posted', 'returned']).optional(),
    occurred_at: z.string().optional(),
  }),
};

type ColumnEvent = keyof typeof schemas;
...
export default async function handler(req: any, res: any) {
  ...
  const event = (Array.isArray(req.query?.event) ? req.query.event[0] : req.query?.event) as ColumnEvent | undefined;
  if (!event || !(event in schemas)) {
    return res.status(404).json({ error: 'Unknown Column webhook event' });
  }

  logAdapterSummary();

  const rateKey = `${req.headers['x-forwarded-for'] || req.socket.remoteAddress || 'unknown'}:column:${event}`;
  ...
  try {
    ...
    if (event === 'auth_request') {
      ...
      await metrics.record({
        ...
        metadata: { source: 'column', event_type: event },
      });
